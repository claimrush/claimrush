...
This file is the **docs-only canonical deployment manifest** for ClaimRush v1.0.0 on local.

Identity:
- Deployment name (manifest selector): `local`
- Chain ID: `31337`
- Explorer: (not specified)

Constraints (required):
- The runtime source of truth is `deployments/local.json` (repo root), which the app loads at runtime.
...
- For local manifests, startBlock MAY be `0` on ephemeral chains, but generated mirrors should be refreshed from the latest deployment output.
...
|---|---:|---:|
| DelegationHub | `0x0DCd1Bf9A1b36cE34237eEaFef220932846BCD82` | `3` |
