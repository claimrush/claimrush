...
DEPLOYMENTS_MANIFEST="$ROOT_DIR/deployments/base_mainnet.json"
...
echo "==> Activating PROD manifest"
cp "$SUBGRAPH_DIR/subgraph.prod.yaml" "$MANIFEST_ACTIVE"

echo "==> Syncing addresses + start blocks from deployments/base_mainnet.json"
python3 "$SYNC_SCRIPT" \
...
# Extra prod guard: require explicit confirmation.
if [[ "${CONFIRM_PROD:-}" != "YES" ]]; then
  echo "Refusing to deploy PROD without CONFIRM_PROD=YES" >&2
  exit 1
fi
...
# Reused patterns across providers.
VERSION_LABEL_RE='^v?[0-9]+\.[0-9]+\.[0-9]+(-[a-z0-9.]+)?$'
SLUG_RE='^[a-z0-9][a-z0-9_/-]{0,63}$'

# ---- Subgraph Studio (The Graph) ----
if [[ "$DEPLOY_STUDIO" == "1" ]]; then
  echo "==> Deploying to Subgraph Studio (prod)"
  ...
  SUBGRAPH_STUDIO_SLUG="${SUBGRAPH_STUDIO_SLUG:-claimrush-v1-0-0}"
  ...
  if ! "$GRAPH_BIN" auth "$SUBGRAPH_STUDIO_DEPLOY_KEY"; then
    echo "ERROR: Subgraph Studio authentication failed. Check SUBGRAPH_STUDIO_DEPLOY_KEY." >&2
    exit 1
  fi
...
if [[ "$DEPLOY_GOLDSKY" == "1" ]]; then
  echo "==> Deploying to Goldsky (prod)"
  require_cmd goldsky
  ...
  GOLDSKY_SUBGRAPH_SLUG="${GOLDSKY_SUBGRAPH_SLUG:-claimrush}"
  if [[ -z "${GOLDSKY_VERSION_LABEL:-}" ]]; then
    echo "Missing required env var: GOLDSKY_VERSION_LABEL (e.g. 1.0.1; Goldsky versions diverge from Studio)." >&2
    exit 1
  fi
...
if [[ "$DEPLOY_GOLDSKY" == "1" ]]; then
  ...
  echo "  Update SUBGRAPH_DIRECT_URL in frontend/wrangler.json (prod) + SUBGRAPH_URL on worker/chat + worker/chat/jobs."
fi
