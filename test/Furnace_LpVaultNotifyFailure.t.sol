// SPDX-License-Identifier: MIT
pragma solidity ^0.8.34;

import {Test} from "forge-std/Test.sol";

import {ClaimToken} from "src/ClaimToken.sol";
import {MineCore} from "src/MineCore.sol";

import {FurnaceHarness} from "./mocks/FurnaceHarness.sol";
import {FurnaceQuoter} from "src/FurnaceQuoter.sol";
import {MockLpRewardsVault} from "./mocks/MockLpRewardsVault.sol";
import {MockShareholderRoyaltiesCheckpoint} from "./mocks/MockShareholderRoyaltiesCheckpoint.sol";
import {VeClaimNFTHarness} from "./mocks/VeClaimNFTHarness.sol";

/// @notice Regression tests: LP rewards vault failures MUST NOT brick Furnace flows.
/// @dev The Furnace streams CLAIM to the LP vault then calls notifyRewards(). If notifyRewards reverts
///      (misconfig, allowlist mismatch, paused vault, upgrade bug), Furnace MUST swallow the error.
contract FurnaceLpVaultNotifyFailureTest is Test {
    ClaimToken internal claim;
    VeClaimNFTHarness internal ve;
    MineCore internal mineCore;
    FurnaceHarness internal furnace;
    MockLpRewardsVault internal lpVault;

    address internal owner = address(0xA11CE);
    address internal mineMarket = address(0xB0B0);
    address internal registry = address(0xE777);

    function setUp() public {
        vm.etch(registry, hex"00");
        claim = new ClaimToken(owner);
        ve = new VeClaimNFTHarness(address(claim), owner);
        address mockSR = address(new MockShareholderRoyaltiesCheckpoint());

        mineCore = new MineCore(address(claim), address(ve), mockSR, owner);
        furnace = new FurnaceHarness(address(claim), address(ve), owner);
        FurnaceQuoter quoter = new FurnaceQuoter(address(furnace));

        vm.etch(mineMarket, hex"00");
        vm.startPrank(owner);
        claim.setMineCore(address(mineCore));
        furnace.setMineCore(address(mineCore));
        furnace.setFurnaceQuoter(address(quoter));
        furnace.setShareholderRoyalties(mockSR);
        furnace.setMineMarket(mineMarket);
        furnace.setEntryTokenRegistry(registry);
        vm.stopPrank();

        lpVault = new MockLpRewardsVault();
        lpVault.setFurnace(address(furnace));
        lpVault.setRevertOnNotify(true);
        vm.prank(owner);
        furnace.setLpRewardsVault(address(lpVault));
    }

    function testAccrueLpStreamDoesNotRevertWhenNotifyReverts() public {
        uint256 fundAmount = 1000 ether;
        deal(address(claim), address(furnace), fundAmount);

        furnace.exposedFundLpStream(fundAmount);

        (uint256 ratePerSec,, uint256 lastUpdate,) = furnace.getLpStreamState();
        assertGt(ratePerSec, 0);
        assertEq(lastUpdate, block.timestamp);

        vm.warp(block.timestamp + 1 days);

        uint256 beforeBal = claim.balanceOf(address(lpVault));
        uint256 streamed = furnace.exposedAccrueLpStream();
        uint256 afterBal = claim.balanceOf(address(lpVault));

        assertGt(streamed, 0);
        assertEq(afterBal - beforeBal, streamed);
    }

    function testFundLpStreamDoesNotRevertWhenAccrueNotifyReverts() public {
        uint256 fundAmount = 1000 ether;
        deal(address(claim), address(furnace), fundAmount * 2);

        furnace.exposedFundLpStream(fundAmount);

        // Advance time so some amount is owed.
        vm.warp(block.timestamp + 1 days);

        uint256 beforeBal = claim.balanceOf(address(lpVault));

        // This call accrues the existing stream first (which triggers notifyRewards).
        furnace.exposedFundLpStream(123 ether);

        uint256 afterBal = claim.balanceOf(address(lpVault));
        assertGt(afterBal, beforeBal);
    }

    function testCreditReserveDoesNotRevertWhenNotifyReverts() public {
        uint256 fundAmount = 1000 ether;
        deal(address(claim), address(furnace), fundAmount);

        furnace.exposedFundLpStream(fundAmount);

        vm.warp(block.timestamp + 1 days);

        uint256 beforeBal = claim.balanceOf(address(lpVault));

        // creditReserve() accrues the overflow drip, which accrues the LP stream first.
        vm.prank(address(mineCore));
        furnace.creditReserve(1 ether);

        uint256 afterBal = claim.balanceOf(address(lpVault));
        assertGt(afterBal, beforeBal);
    }
}

